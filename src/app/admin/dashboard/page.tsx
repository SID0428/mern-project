...
import { Upload, FileText, Building2, Download } from 'lucide-react';
import { uploadCourses } from '@/lib/api';
import { useAppContext } from '@/context/app-context';

function UploadCard({ title, description, onUpload, onDownloadTemplate }: { title: string, description: string, onUpload: (file: File) => void | Promise<void>, onDownloadTemplate?: () => void }) {
...
export default function AdminDashboardPage() {
  const { toast } = useToast();
  const { auth, refreshCourses } = useAppContext();

  const handleUniversityUpload = (file: File) => {
    toast({
      title: 'Success',
      description: `University data from "${file.name}" selected. Course upload is connected to the backend API.`,
    });
  ...
  const handleCourseUpload = async (file: File) => {
    if (!auth?.token) {
      toast({
        ...
        description: 'Please sign in before uploading course data.',
      });
      return;
    }

    try {
      const result = await uploadCourses(file, auth.token);
      await refreshCourses();
      toast({
        title: 'Success',
        description: `Imported ${result.imported} courses from "${file.name}".`,
      });
    } catch (error) {
      toast({
        variant: 'destructive',
        title: 'Upload Failed',
        description: error instanceof Error ? error.message : 'Could not upload course CSV.',
      });
    }
  };
    ...
    <div className="container mx-auto py-12 px-4">
      ...
      {!auth && (
        <div className="mb-6 rounded-md border border-destructive/40 bg-destructive/10 p-4 text-sm text-destructive">
          Sign in with an admin account to upload course CSV data.
        </div>
      )}
