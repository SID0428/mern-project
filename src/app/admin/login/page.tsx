...
export default function AdminLoginPage() {
  ...
  const handleSignup = async () => {
    setIsLoading(true);
    try {
      const auth = await signupAdmin(email, password);
      setAuth(auth);
      toast({
        title: 'Admin Created',
        description: 'Your JWT session has been saved locally.',
      });
      router.push('/admin/dashboard');
    } catch (error) {
      toast({
      ...
      <Card className="w-full max-w-md">
        <form onSubmit={handleLogin}>
          ...
          <CardFooter className="flex flex-col gap-3">
            ...
            <Button type="button" variant="outline" className="w-full" disabled={isLoading} onClick={handleSignup}>
